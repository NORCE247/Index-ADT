 ...
 * Therefore the struct has to keep track of what the current result is, and
 ...
 * @struct search_hit
 *
 * @warning You can add any member to this struct, but do not remove the
 * existing members.
 *
 ...
 * If the index_t struct contains other structures (lists or hashtables etc.)
 ...
 * Any word that are contained in the index, should also be deallocated here (or
 * in another appropriate function).
 */
 ...
 * @param[in] words A linked list containing all the words and special
 * characters in the document.
 *
 * The function adds the content of a new document to the index.
 * The name of the document is not necessarily required, but could be preserved.
 * It will be deallocated by the caller after the function.
 *
 * The supplied list of words will be deallocated after this function, but not
 * the words. The callie is responsible for deallocating the words after use.
 */
 ...
 *
 * The function can *either* search for a continuous sentence, or each word
 * individually. If the function searches for sentences, then it should be an
 * exact match between the query and the result. Otherwise the spaces should be
 * treated as separators between query words.
 *
 * The function searches for the query accross multiple documents.
 * The return value must handle iteration of all search hits accross all
 * documents.
 *
 ...
 * @param[in] size The length of the input string, excluding the
 * null-terminator.
 * @return char* A null-terminated string containing the word closest to the
 * input. NULL if no word is found.
 *
 * Completes a word starting with the string input. The return value should be
 * the whole word and must be null-terminated.
 */
 ...
 * @return char** An array of char arrays containing the words of the current
 * document. NULL if there are no more documents.
 *
 * Since there are multiple documents in the index, this function returns the
 * content of the document pointed to by the struct. Subsequent calls to this
 * function should return the content for the next document, ie the function
 * should only be called once per document. The function is equivalent to
 * calling a next function for documents. The function returns NULL when there
 * are no more documents in the search result.
 */
 ...
 * Length is the number of words that has been tokenized from the original file.
 ...
 *
 * @param[in,out] res The result context (iterator).
 * @return search_hit_t* A pointer to the current search hit. Returns NULL when
 * there are no more word hits.
 *
 * The function should return a @sa search_hit_t struct, which represent the
 * current search hit (location and length) in the current document. Subsequent
 * calls to this function advances the iterator. The function is equivalent to a
 * next function for search hits. The function returns NULL when there are no
 * more search hits in the current document.
 *
 * To get the search results of the next document, result_get_content must be
 * called first.
 *
 */
