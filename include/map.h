...
/**
 * @brief Destroys the given map. Will also destroy keys and values if functions
 * are supplied.
 *
 * @param map The map to destroy.
 * @param destroy_key A pointer to a function to destroy the keys. Can be NULL.
 * @param destroy_val A pointer to a function to destroy the values. Can be
 * NULL.
 *
 * The function will destroy keys if the supplied destroy_key function is
 * supplied. The function will destroy values if the supplied destroy_values
 * function is supplied. If the arguments are NULL, the respective field is not
 * touched.
 */
void map_destroy(map_t *map, void (*destroy_key)(void *),
 ...
 * @return void* A pointer to the element that the key is mapped to. NULL if the
 ...
 *
 * Hash algorithm devised by Daniel J. Bernstein.
 * NOTE: This has to be changed to suit the datatype used by the hash table.
 */
