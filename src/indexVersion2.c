...
// Function prototypes
static search_result_t *index_multi_find(index_t *idx, list_t *tokens,
                                         const char *query);
...
typedef struct search_result {
  ...
  index_t *index; ///< A pointer to index_t, where its have data that represent
                  ///< a document.
...
static search_result_t *cmpSearchResult(search_result_t *main,
  ...
  if (main == NULL || sub == NULL || main->hitsList == NULL ||
      sub->hitsList == NULL) {
    return NULL;
  }
