 ...
 * Special characters and valid words are extracted and placed in the given list
...
void parse_word(char *token, list_t *l) {
  ...
  while (*start != '\0') {
    ...
    if (special_char ==
        start) // Encountered special character at the start of the token
    {
      // Allocate len + 1, which will null terminate the string when using
      // calloc
      ...
      special_char = strpbrk(start, match_chars);
               ...
               start) // Encountered special character at the end of the token
    {
...
int compare_strings(void *a, void *b) { return strcmp(a, b); }
...
unsigned long long gettime() {
  ...
  // Convert the seconds to microseconds and add in the microsecond part of the
  // time
