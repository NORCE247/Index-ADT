...
int trie_insert(trie_t *trie, char *key, void *value) {
  ...
  /**@MODIFIED: */ /* len of char is needed to define the number of leftover
                      chars when insert is done. */
  int len = strlen(key);
  ...
  for (int i = 0; key[i] != '\0'; i++) {
    ...
    *nr = len - i;
    ...
    /** @MODIFIED: **/ /* Overwrites the node_t value to represent the number of
                          leftover chars. */
    iter->value = nr;
...
char *trie_find(trie_t *trie, char *key) {
  ...
  for (int i = 0; key[i] != '\0'; ++i) {
    ...
    if (ASCII_TO_IDX(tolower(key[i])) < 0 ||
        ASCII_TO_IDX(tolower(key[i])) > 25) {
      return NULL;
    }

    /* Update the current node to represent the node with the current
     * character*/
