...
typedef struct search_result {
  ...
  index_t *index;   ///< A pointer to index_t, is used to store the words in a
                    ///< document where search results is founded
  search_result_t
  ...
  int accessedCounter; ///< Represent the number of times a file have been
                       ///< visited. [0,1,2,3] = [create, content, length, next
                       ///< content]
...
search_result_t *index_find(index_t *idx, const char *query) {
  ...
  if (idx == NULL || idx->documentName == NULL) {
    return NULL;
  }
  ...
  if (found) {
    ...
    /* If there is more documents, use recursion to store the next search
     * results */
    if (idx->next != NULL) {
