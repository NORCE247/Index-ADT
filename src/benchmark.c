...
const char *example[15] = {"\"", "Lorem",      " ", "ipsum", " ", "dolor",
...
int main(int argc, char **argv) {
  ...
  fprintf(stdout, "N Words|Time(µs)|Time/Word(µs)|Search Hit Time(µs)|Search "
                  "Miss Time(µs)\n");
  for (int i = 0; i < nruns; i++) {
    ...
    after = gettime();
    fprintf(stdout, "%d %llu %lf ", nwords, after - before,
            (double)(after - before) / nwords);
  ...
  TEST_PRINT(
      "Average time to find a single that does not exist (miss) word: %lf µs\n",
