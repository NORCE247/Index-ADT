...
void PRINT_DEBUG_INFO(int inpos, int spos, int cur_word_len, char *input,
                      char *suggestion) {
  int x, y;
...
void ui_init(void) {
  ...
  // Enables interpretation of function keys
  // instead of having to interpret escape sequences.
  keypad(stdscr, TRUE);

  // Sets up a few color pairs
  start_color();
  init_pair(1, COLOR_BLACK, COLOR_CYAN);
  init_pair(2, COLOR_BLACK, COLOR_WHITE);
  init_pair(3, COLOR_BLUE, COLOR_WHITE);

  // Set beckground and foreground color
  if (wbkgd(stdscr, COLOR_PAIR(3)) != OK) {
    fprintf(stderr, "Could not set the background color\n");
    exit(1);
  }
...
/**
 * Display the current input with suggestions if any.
 */
...
/**
 * Main UI loop.
 * This handles input from the user and returns it as a char *
 *
 */
char *ui_main(index_t *idx) {
  ...
  while ((c = getch()) != '\n') {
    ...
    if (inpos >= 200) {
      ui_display_input("Too long input, only 200 characters allowed.", NULL,
                       cur_word_len);
      input[200] = '\0';
      return input;
    }
    ...
    case 127:
      ...
      if (inpos > 0) {
        ...
        // If we encounter a space char, we need to find the start of the
        // previous word
        if (input[inpos] == ' ') {
          spos = inpos;
...
static void ui_display_results_content(char **content, int content_length,
                                       search_hit_t *cur_pos) {
  move(1, 0);
  int from = 0;
  int y;
